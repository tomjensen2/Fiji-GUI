...
function MATLABCallOriginMatrix(Obj,Channel,worksheetName)
...
%     wkm = originObj.FindMatrixSheet(strmat);
    ...
    data=Obj.(Channel);
    if Obj.Type=="Line2"
        mdata=zeros(size(data)+1);
        mdata(2:end,2:end)=data;
        mdata(1,2:end)=Obj.TData;
        mdata(2:end,1)=Obj.YData.';
    end
    % Set 3 columns
    wks.Cols = size(mdata,2);
    wks.Rows = size(mdata,1);
    wks.SetData(mdata)
