...
%% 任梦达新增:持久位置记录
persistent FigPos_mem
%%
if ~nargin
    Pos = [ 0.6 , 0.1 ];
   Question = 'Is the position proper?';
   Title = 'Is it OK?';
end
% MFQUESTDLG Question dialog box.
% ----------------------------------------------------------------------- 
%  --------------------- Modified Function Help ----------------------
%  慚Fquestdlg?is a simple modification of 憅uestdlg?which enables user
%  to specify the position of 'questdlg' question dialog box on the screen.
%  Everything is like the original 憅uestdlg? except that you need to determine
%  your desirable position with a two-element vector of the form:
%  [left, bottom]
%  where 憀eft?and 慴ottom?define the distance from the lower-left corner
%  of the screen to the lower-left corner of the question dialog box.
%  憀eft?and 慴ottom?are NORMALIZED units, obviously between 0 and 1.
%  The position vector must be added as the first element of 慚Fquestdlg?
%  Example:
...
%% 任梦达新增: 随时微调对话框位置
if ~isempty(FigPos_mem) % || ~isequal(FigPos_mem,FigPos)
    % 第二次起,忽略位置参数,而是选择永久参数
    FigPos = FigPos_mem;
else
    % 首次运行,按照参数赋值给永久记录
    FigPos_mem = FigPos;
...
%% 任梦达新增: 让对话框永远在最前,哪怕不是焦点,
% 试图让面板显示在最前,这样即便遮挡部分imagej的画板,我画GUI时也不会掉落到底层!
% 这样就方便我画完roi然后葛优躺一样悠悠然点击近在咫尺的飘浮的面板按钮!
if 0 % 手动调试bug
    % gui非阻塞.注意,normal下非阻塞,设置为最前也没有用!
    set(QuestFig ,'WindowStyle','normal','Visible','on');
    assignin('base','QuestFig',QuestFig);
else
    % gui阻塞,这样设置为最前才有效果
    set(QuestFig ,'WindowStyle','normal','Visible','on');
end
% 在获取对话框句柄并修改java属性前,一定要更新,要不然根本没显示,也就抓取不到!
drawnow;
% 获取同名的窗口
frames = java.awt.Frame.getFrames();
for i = 1 : length(frames)
    aTitle = get(frames(i),'Title');
    if isequal(aTitle, Title)
        frames(i).setAlwaysOnTop(1);
        % 千万不能break,因为一个窗口有时有好几个frame对象在支撑,你不知道那个是负责永远在最前的
%         break
    end
end
%%
...
if ishghandle(QuestFig)
  ...
  uiwait(QuestFig);
  %% 任梦达新增: 每次用户手动微调后,获取用户当时喜欢的position
      % 首次运行,按照参数赋值给永久记录
      FigPos_mem = QuestFig.Position;
end
...
%{
Another new feature: 
Sometimes in a loop we do something then click the dialogue.
If the working area of our focus is large, such as we draw an roi on image, 
we need to position the dialogue to the far side of screen because 
if position on top of image, when we work on image the dialogue will be hidden to bottom. 
It would be annoying that we need to activate the dialogue again and again from back of screen.
Now with this new feature, the dialogue will always floating on top of screen and it will not affect your action on images.
After draw an roi, you can click the dialogue just at side 
(if you use a touchscreen and use your finger to draw roi, then it feels much better)..
What’s more, if combine my previous feature posted just now, 
you can change your mind of the position of dialogue by finger or mouse anytime and it take effect immediately the next call.

Cool!
----how to add this feature: just add the following lines before the “drawnow” line:
% 在获取对话框句柄并修改java属性前,一定要更新,要不然根本没显示,也就抓取不到!
drawnow;
% 获取同名的窗口
frames = java.awt.Frame.getFrames();
