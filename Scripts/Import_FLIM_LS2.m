function dataout =Import_FLIM_LS2(currentobj,filename,channel,importtype)
...
%         button=questdlg('Is there a matching Red Channel?','Yes or No','Yes','No','Yes');
%         switch button
%             case 'Yes'
%             dataout=cell(size(file,2),2)
%             case 'No'
%             dataout=cell(size(file,2),1)   
%         end
        

% currentfile=filenames(1,filenumber);
% filename=currentfile{1,1}
status=false;
...
%                    binsize=[2,1,1,1,1] 
    ...
    assignin('base','file_convert_didn_t_work','file_convert_didn_t_work')
end

clearvars -except dataout filename filenumber channel newrawdata currentobj importtype t_new
...
switch importtype
    case 3
        if size(currentobj.scanline.roi,2)==1
           if channel==1
            currentobj.UG=squeeze(nansum(newrawdata,2))
            currentobj.comment=sprintf('%s_%s_%s','FLIM','UG',currentobj.comment)
            currentobj.Subtype='Single'
           ...
           currentobj.comment=sprintf('%s_%s_%s','FLIM','UR',currentobj.comment)
           currentobj.Subtype='Single'
            currentobj.comment=sprintf('%s_%s','tau/Time',currentobj.comment) 
            currentobj.XData=linspace(0,max(currentobj.TData,[],'all'),size(currentobj.UR,2));
            currentobj.YData=t_new;
            currentobj.TData=linspace(0,max(currentobj.TData,[],'all'),size(currentobj.UR,2));
            currentobj.tauData=t_new;
            currentobj.XDim=2;
            currentobj.TimeDim=2;
            currentobj.YDim=1;
