function [H,Points,inside_outer] = kscontour2(X, varargin)
...
%   PrcHi - upper percentile for bound on density estimate (default=95)
%
% OUTPUT
...
addParameter(p, 'PrcHi', 95, @isscalar);
addParameter(p, 'Handle', 98, @isobject);
addParameter(p, 'Replace', 98, @islogical);
parse(p, varargin{:});
...
if iscell(X)
    ...
    for iX = 1:length(X)
        H(iX) = kscontour2(X{iX}, 'Color', colors(mod(iX-1,8)+1,:), ...
...
while iC<size(C,2)
    ...
    if isobject(handle)==1
    hold(handle,'off') 
   
    H = patch(handle,C(1,ix), C(2,ix), max(0, (Color*256-2*tC)/256), ...
              'FaceAlpha', Alpha, 'EdgeColor', 'none','Tag','kscontour');    
    hold(handle,'on')
    else
    ...
    inside_outer(:,i1)=inpolygon(X(:,1), X(:,2),C(1,ix), C(2,ix));    
    Polys(i1,1)=polyshape(C(1,ix), C(2,ix))
    i1=i1+1;
...
if ShowPoints
    if isobject(handle)==1
    Points=scatter(handle,X(:,1), X(:,2), PointSize, 'filled' , ...
            'MarkerFaceColor', Color,'Tag','kscontour'); 
        hold(handle,'off')
    else    
    Points=scatter(X(:,1), X(:,2), PointSize, 'filled' , ...
            'MarkerFaceColor', Color,'Tag','kscontour');
    end
end
