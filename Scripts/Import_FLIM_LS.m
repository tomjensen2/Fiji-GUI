function dataout =Import_FLIM_LS()
...
[file,folder]=uigetfile('*.spc','MultiSelect','on')
a=0
...
%         end
if iscell(file)==1
filenames=cellfun(@(x) cat(2,folder,x),file,"UniformOutput",false)
else 
filenames={cat(2,folder,file)}
end
outputfiles=file
progressbar
...
dataout(filenumber,1)={newrawdata}
clearvars -except dataout filenames filenumber
progressbar(filenumber/numel(filenames))
end
%  ask if want auto load
button=questdlg('Is there a matching Red Channel?','Yes or No','Yes','No','Yes');
if button=="Yes"
...
tempfilename=filenames{1,i};
tempfilename(end-8)='2'
filenames(1,i)={tempfilename}
end
clearvars -except dataout filenames 
 for filenumber=1:numel(filenames)
currentfile=filenames(1,filenumber);
filename=currentfile{1,1}
status=false;
...
dataout(filenumber,2)={newrawdata} 
clearvars -except dataout filenames filenumber newrawdata
progressbar(filenumber/numel(filenames))
 end
